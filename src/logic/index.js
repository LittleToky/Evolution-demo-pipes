...
export const getNeighbourCellData = (coords: Vector2, data: Cell[][], liveData:Boolean[][], direction: Vector2) => { // coords - of current node, direction - from current node to neighbour
  const [x, y] = direction;
  ...
  const responsing = neighbourCell ? // (neighbour cell exists)
    !!neighbourCell.connections.find(([xTemp, yTemp]) => (xTemp === -x) && (yTemp === -y)) : // check if it has responsive connection to the current cell
